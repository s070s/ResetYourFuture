...
GitHub Actions runs on every push and pull request: a locked-mode restore (see the Dependabot exception below), a Release build, a **vulnerable-package gate** (`dotnet list package --vulnerable --include-transitive` — any advisory, including a transitive one, fails the build), a **code-style gate** (`dotnet format style --verify-no-changes` — a file that drifts from `.editorconfig` fails the build), and `dotnet test`, then uploads TRX results from `TestResults/*.trx`. A separate job applies the full migration chain against a real SQL Server container to catch migrations that pass on InMemory/SQLite but break on SQL Server. There is no coverage gate or coverage artifact yet; the definition of done is a green `dotnet test` locally and in CI. Style and the recommended analyzers also run in every local build (`EnforceCodeStyleInBuild`), surfacing as warnings rather than blocking day-to-day work.

> **Dependabot lock-file refresh (DEP-10):** Dependabot bumps `Directory.Packages.props` but never regenerates the `CentralTransitive` entries in the ten `packages.lock.json` files, so its PRs would all fail the locked-mode restore with `NU1004`. On `dependabot/*` branches only, CI restores with `--force-evaluate` instead and — after the build, style gate and tests have passed — commits the refreshed lock files back to the PR branch. Every other branch keeps the strict `--locked-mode` guard. If a bump exposes a genuine version conflict (`NU1605`, e.g. a transitive floor above a direct pin), the restore still fails and needs a human to adjust the pin.
>
> Expect the PR to end up **"unstable"** rather than fully green: GitHub queues the run for CI's own commit in an `action_required` state that needs a manual "Approve and run" click. That queued run is redundant — its tree is identical to the one the preceding green run built and tested — so the PR is safe to merge as-is. Approving it (or clicking it away) is optional.
