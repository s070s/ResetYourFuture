...
### ARCH-1: SSR renders through a loopback HTTP call to the app's own API — the costs are systemic, the benefits real but demo-scoped  [High] [Effort: L]
- **Evidence:** `src/ResetYourFuture.Web/Startup/ServiceRegistrationExtensions.cs:203-254` (19 typed `HttpClient` consumers + 1 named client, all pointed at `SelfBaseUrl` defaulting to `https://localhost:7090`; the code comment at lines 204-210 itself warns that in production SelfBaseUrl must match the bound address AND the loopback TLS cert must be trusted, "otherwise every API-backed page silently renders empty"); `src/ResetYourFuture.Web/Consumers/ApiClientBase.cs:41-121` (every helper returns `default` on any non-success status — the silent-empty mechanism); `src/ResetYourFuture.Web/Services/SsrApiHandler.cs:35-55` (a fresh JWT is minted and signed per outbound request); `src/ResetYourFuture.Web/Startup/AuthenticationSetupExtensions.cs:159-180` (`OnTokenValidated` does a `UserManager.FindByIdAsync` DB lookup for every one of those loopback calls).
- **Impact:** Every page render pays: JSON serialization both ways, an extra HTTP+TLS round trip, a JWT sign, a JWT validate, and a per-call user DB lookup — multiplied by however many consumers a page touches (PERF 34 owns quantification). Failures are silent by design (`default` → blank page section, no error surfaced). Deployment is coupled to `SelfBaseUrl` and loopback certificate trust — the first non-dev deployment will hit this. Cross-consumer operations can never share a DB transaction. **What it buys (honest ledger):** the API cannot rot or go incomplete because the app itself is its first consumer; there is exactly one authorization surface (controllers + policies) instead of a UI path and an API path that drift; DTO discipline is forced at every boundary; the Swagger/OpenAPI document describes the whole product; and `WebApplicationFactory` integration tests exercise the same path users do. For a university certificate project whose brief includes "a documented REST API", this is a defensible — arguably clever — tradeoff.
- **Recommendation:** Keep it for the certificate, but (1) record the decision and its production caveats as a short ADR instead of only a code comment (see MAINT 23 on comment-trapped knowledge); (2) the consumer interfaces (`ICourseConsumer` etc.) already isolate the transport — if this ever heads toward real deployment, provide in-process implementations of the same interfaces that call the Application services directly and swap them by DI registration, exactly as the code comment at `ServiceRegistrationExtensions.cs:210` suggests; (3) make the failure mode less silent (ApiClientBase logging a warning on non-success would cost one line per helper — REL 26 owns the runtime behavior).
...
|----|----------|--------|--------|
| ARCH-1 | High | L | Write an ADR for the loopback self-API design; plan in-process consumer implementations behind the existing interfaces as the production path; de-silence ApiClientBase failures |
