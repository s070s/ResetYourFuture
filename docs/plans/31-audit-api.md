...
- **Evidence:** `src/ResetYourFuture.Application/DTOs/PagedResult.cs:11-12` — the generic envelope still defaults `SortBy = "email"`, `SortDir = "asc"`. The sorting rollout wired real sort parameters through the admin/history endpoints, but the constructors that don't sort still omit the arguments and inherit the defaults (e.g. `ChatQueryService.cs:53,93,138`, `CourseService.cs:107` student course list, `AssessmentService.cs:68` student assessment list), so chat, student-course and student-assessment lists all serialize `"sortBy": "email"` — a field none of them sorts by or even contains.
- **Impact:** Actively false response metadata; a client that echoes the envelope's sort state back (the intended pattern for the sorted grids) would send nonsense to the unsorted endpoints.
- **Recommendation:** Make `SortBy`/`SortDir` nullable with no defaults (omit when the endpoint doesn't sort), or move them to a derived `SortedPagedResult<T>` used only by endpoints that implement sorting.
...
- **Server-side sorting (former plan 10, implemented):** the rollout landed across the admin/history list endpoints; API-6 only covers the envelope's false metadata on the endpoints that don't sort.
