...
For ~90 endpoints built incrementally, the API surface is in good shape where it was designed deliberately: routes are consistently `api/{resource}` with admin surfaces under `api/admin/*`, every controller carries `[Tags]` grouping, list endpoints share a real `PagedResult<T>` envelope with clamped paging, `AddProblemDetails` + `UseStatusCodePages` + a production exception handler give bare status codes a uniform RFC 7807 body with `traceId`, and the OpenAPI pipeline is unusually mature (per-operation bearer requirements, curated parameter docs, request examples, SSE and SignalR documented in prose, minimal APIs included with summaries). The dominant weakness is the *error* half of the contract: five different error body shapes coexist, uniqueness conflicts map to different status codes per controller, `[ApiController]` auto-validation silently supersedes the hand-written ModelState envelopes, and a handful of documented response types don't match what actions actually return.

## 3. Findings

### API-1: Error responses have at least five competing body shapes (and two content types)  [High] [Effort: M]
- **Evidence:** (a) RFC 7807 ProblemDetails — only for *bare* status results and unhandled exceptions (`ServiceRegistrationExtensions.cs:136-140`, `Program.cs:42-65`). (b) Raw string bodies, serialized as `text/plain` by MVC's StringOutputFormatter: `ServiceResultExtensions.cs:16-25` returns `result.ErrorMessage` (a string) as the ObjectResult body for every ServiceResult failure; plus dozens of literal-string results (`AdminController.cs:121`, `AdminLessonsController.cs`, `CertificatesController.cs:109-131`, `LessonAssetsController.cs` including a `StatusCode(500, "Error retrieving asset")` catch that bypasses the exception handler). (c) `AuthResponseDto { Success=false, Message/Errors }` from auth endpoints (`AuthController.cs:36,62,78,104`, `AuthApiService.cs:142-145,186-189`). (d) Anonymous objects: `SubscriptionController.cs:83-86` (`{ message }` on 503), `121-144` (`{ error }` on webhook 400s). (e) Outcome-embedded DTOs at error status: `CoursesController.cs:63-64` (403 with `EnrollmentResultDto`), `SubscriptionController.cs:77` (400 returns the `CheckoutSessionDto` itself), `202` (400 with `CancelSubscriptionResultDto`).
- **Impact:** A client cannot write one error handler: the same 400 can arrive as `application/problem+json`, `text/plain`, or three different JSON shapes depending on which endpoint produced it. The Swagger document's class-level `[ProducesResponseType(Status400BadRequest)]` declarations carry no schema, so the docs can't warn consumers either. The `text/plain` string bodies also defeat the `traceId` correlation that ProblemDetails was configured to provide.
- **Recommendation:** Make ProblemDetails the single error envelope: change `ServiceResultExtensions.ToActionResult` to emit `ProblemDetails { Status, Detail = ErrorMessage }` (one change fixes every ServiceResult consumer), replace literal-string `BadRequest("...")`/`NotFound("...")` with `Problem(statusCode:, detail:)`, and keep `AuthResponseDto` for *success* payloads only. The auth endpoints can carry machine-readable failure codes in a ProblemDetails extension instead of `Success=false`.
...
|----|----------|--------|--------|
| API-1 | High | M | Standardize all error bodies on ProblemDetails: rewrite `ToActionResult`, eliminate string-bodied error results, keep DTO envelopes for success only |
