...
- **Recommendation:** Add "verify SMTP egress from the host (587 STARTTLS)" to `docs/DEPLOYMENT.md`.
...
- **40 (BUILD)** — container publish target (BUILD-7) that could make `docs/DEPLOYMENT.md` (former CLOUD-5) executable via a compose file; CI SQL Server container (BUILD-2).
