...
> **Fixed since audit:** UX-1 (High — ~100 user-facing strings hardcoded in English, so action feedback ignored the selected language, and `$"Error: {ex.Message}"` leaked raw exception text) — swept the shared-component English defaults (ConfirmModal, FormModal, DismissibleAlert, PaginationNav, StatusBadge, LoadingSpinner, PageStateContainer, UpgradePrompt, NavMenu, the `#blazor-error-ui` banner) onto `GlobalRes` keys, the eight literal `ItemLabel` nouns onto `AdminRes.ItemLabel*`, and every hardcoded success/failure/validation message across the auth, student and admin code-behind onto `ErrorMessagesRes`/`SuccessMessagesRes`/`AdminRes` (66 new keys total, full EL parity). Every `$"...{ex.Message}"` now shows a generic localized message (the auth pages also log the exception). Verified live in both cultures. (Adding loggers to the ~13 admin catch blocks that never logged is left to REL's silent-swallow finding — this sweep's scope was language + stopping the leak.)

> **Fixed since audit:** UX-3 (High — unknown routes rendered a blank page) — Blazor's `<NotFound>` render fragment was removed in .NET 10 in favor of `Router.NotFoundPage`; the fix is a routable `Pages/NotFound.razor` wired via `NotFoundPage="typeof(NotFound)"` plus `app.UseStatusCodePagesWithReExecute("/__status-code-dispatch")` in `Program.cs` (a dispatcher endpoint that keeps `/api/*` on its JSON `ProblemDetails` contract and renders the Blazor page for everything else). UX-4 (High — unconfirmed subscription downgrade) — `Pricing.razor`'s downgrade button now goes through the same `ConfirmModal`/`BillingRes` strings as Billing's cancel flow. UX-2 (High — silent assessment submission failures, unenforced required questions) — `AssessmentForm` now tracks `_loading`/`_loadError`/`_submitError` state (error+Back on load failure, an alert on submit failure) and validates `Required` questions client-side before posting, marking missing fields `is-invalid` and blocking submission with a localized message.
