...
|----|----------|--------|--------|
| MAINT-2 | Medium | M | Automate resx→code generation (verified NuGet source generator or IStringLocalizer); MSBuild-only attempt tried and reverted |
| MAINT-3 | Medium | M | Generate consumers from the OpenAPI doc; constants-only alternative assessed and set aside |
