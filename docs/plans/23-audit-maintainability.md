...
> **Fixed since audit:** MAINT-1 (High — one new field ripples through 7–9 artifacts across four projects) — addressed per its own recommendation: accept the layer count (the ARCH-1 tradeoff), shrink the per-layer cost. (a) A shared mapping layer now exists at `src/ResetYourFuture.Application/Mappings/` — one static class per entity family, with `Expression`-returning members for EF in-query projections (language-parameterized where needed) and matching materialized extensions side by side, so a field addition edits one adjacent pair instead of scattered copies. Every DTO that was hand-constructed in 2+ places was consolidated: `AssessmentDefinitionDto` (2 byte-identical 11-field projections), `AssessmentSubmissionDto`, `AdminAssessmentDefinitionDto` (3), `ChatMessageDto` (3 files — the highest drift risk — plus the duplicated call-duration math), `ChatUserDto` (2 files), `AdminModuleDto` (4), `AdminLessonDto` (3), `AdminCategoryDto` (3), `CertificateDto` + `CertificateVerificationDto`, `AdminScheduledSessionDto` (2), `MyCourseReviewDto` (2), and `NotificationDto` (the dispatcher now reuses the promoted helper). Deliberately left alone: the ~40 single-construction-site DTOs (no ripple win), `SiteSearchHitDto` (four one-line maps of *different* entities, already co-located), `ChatConversationDto` (its two sites share only the constructor call, not the resolution logic), and `CallParticipantDto` (`CallRegistry` is deliberately dependency-free). (d) `docs/ADDING-A-FIELD.md` now walks the full ripple — entity → EF config → migration → DTO → mapping helper → controller → consumer → page → resx×3 → tests — with the silent-blank-value failure mode called out. Verified behavior-neutral: full suite green (878 tests) plus a 19-check live API pass exercising every refactored mapper (admin CRUD round-trips, language-resolved projections, post-submit mapping) and the e2e suite covering the chat/call mappers. Steps (b) and (c) remain their own findings (MAINT-2, MAINT-3).

Day-to-day maintainability rests on strong foundations: central package management with a single TFM pin, a six-file startup decomposition that makes Program.cs readable in one screen, rigorously consistent code-behind separation for all non-trivial pages, an `.editorconfig`, and an unusually high standard of *why*-comments that preserve hard-won knowledge (circuit/cookie constraints, EF provider quirks, DI-lifetime rationale). The dominant tax is structural: the loopback self-API design plus hand-written consumers plus hand-maintained localization Designer files means a single new field ripples through seven-to-nine artifacts across four projects — each step simple, but the sum makes small changes slow and easy to leave half-done (the shared mapping layer and the `docs/ADDING-A-FIELD.md` checklist now blunt this — MAINT-1, fixed). Secondary frictions are hand-curated artifacts that must be remembered (Designer.cs, OpenAPI schema list) and operational knowledge that lives only in code comments where a deployer will never see it.
