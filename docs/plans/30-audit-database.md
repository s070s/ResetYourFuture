...
- **Impact:** A student can submit an `AnswersJson` blob up to the DTO's `[MaxLength(50_000)]` bound only, not the column itself — the column is opaque to the DB (no `ISJSON` constraint, no EF `ToJson()` mapping) and has no cap of its own, so anything bypassing the DTO (a future code path, a migration-time edit) isn't bounded. Blob growth still hits backup size and buffer pool. (Content-level validation of `AnswersJson`/`SchemaJson` against their assessment's schema is DQ-2/DQ-4's finding, both fixed; the missing schema-level length caps/typing are the DB-side gap this finding owns.)
