...
- **Recommendation:** Drop the redundant `LessonCompletion.UserId` index; review the single-column bool indexes once real query plans exist (PERF's measurement, DB's schema change). Not urgent — cheap to leave, cheap to fix.
