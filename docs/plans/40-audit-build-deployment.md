...
- **Update (2026-09-09) — the SDK-drift sub-item is closed; the rest stays open.** The Impact above was wrong that "none of it breaks anything today": the drift window it describes closed on master when SDK `10.0.401` shipped, whose bundled ASP.NET Core `10.0.12` no longer matched the `Microsoft.AspNetCore.App.Internal.Assets` `10.0.11` entry in `src/ResetYourFuture.Web/packages.lock.json`, failing both jobs' `--locked-mode` restore with `NU1004`. Fixed by pinning `dotnet-version: 10.0.400` in both jobs rather than by the recommended `global-json-file`, because `global.json` asks for `10.0.100` with `rollForward: latestFeature` and so would have permitted `10.0.401` too — tying CI to it would not have pinned anything. The concurrency group, the push-to-master restriction and the NuGet cache are all still absent.
- **Update (2026-09-12) — the pin needs maintaining, not just setting.** Commit `1513d66` refreshed the `Microsoft.AspNetCore.App.Internal.Assets` lock entry to `10.0.12` without moving the CI pin, so both jobs failed `--locked-mode` with `NU1004` again — the mirror image of the 2026-09-09 break. The pin is now `dotnet-version: 10.0.401` (bundles ASP.NET Core `10.0.12`) in both jobs. Lesson recorded in `README.md`: the pin and the lock entry are a single unit and must move in the same commit.
...
- **Evidence:** *(narrowed 2026-09-05 — the original evidence said `Directory.Build.props` sets no `AnalysisLevel` or `EnforceCodeStyleInBuild` and that CI was restore/build/test only. Both halves were overtaken by GOV-2 and by the DEP-9/DEP-10 work.)* `Directory.Build.props` now sets `EnforceCodeStyleInBuild` and `AnalysisLevel=latest-recommended`, and CI hard-gates the warning-level style rules (`dotnet format style --verify-no-changes --severity warn`). The vulnerable-package gate DEP-9 added was itself later removed when Dependabot was retired (2026-09-05; see report 43) — `NuGetAuditMode=all` still surfaces advisories as restore warnings, but nothing fails the build on them. What is still missing is `TreatWarningsAsErrors`: the Release build step is plain `dotnet build -c Release --no-restore`, so the ~950 recommended-analyzer warnings remain advisory.
