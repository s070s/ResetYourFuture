...
For a solo certificate project, the automated test estate is genuinely strong: ~700 test cases across four layers, real-pipeline integration tests through `WebApplicationFactory<Program>` that exercise the same loopback API the app itself uses, a purpose-built pure state machine (`CallRegistry`) that makes the hardest feature unit-testable, a hand-rolled SignalR hub harness, consumer tests against a stubbed handler, an auth-matrix suite, and CI running everything on every push. The two structural holes are at the edges of that estate: everything relational is simulated (EF InMemory carries the integration suite, so unique-index races, LIKE translation, and migrations are never exercised together with the pipeline), and everything browser-side is untested (no e2e and no component tests, leaving the Blazor circuit — where this app's trickiest behavior lives — entirely to manual verification). One production decision made *for* testability (storing `DateTimeOffset` as strings so SQLite can sort) deserves a conscious revisit.

## 3. Findings

### TEST-1: The integration suite runs on EF InMemory — the relational behaviors production relies on are never integration-tested  [High] [Effort: M]
- **Evidence:** `tests/ResetYourFuture.Web.Tests/CustomWebAppFactory.cs:63` (`UseInMemoryDatabase`) carries all 18 integration test classes. Production code paths that depend on relational semantics: `src/ResetYourFuture.Application/ApiServices/CourseService.cs:201-217` (enrollment race handled by catching `DbUpdateException` from the unique `(UserId, CourseId)` index — InMemory enforces no unique indexes, so this path can only be hit in production); `CourseService.cs:40-43` (`EF.Functions.Like` search); the soft-delete query filters in `src/ResetYourFuture.Infrastructure/Data/ApplicationDbContext.cs:107-129`; and migrations themselves — `Startup/DatabaseSeedingExtensions.cs:46-47` guards `MigrateAsync()` behind `IsRelational()`, so no test ever applies the migration chain. A relational escape hatch exists (`tests/ResetYourFuture.TestSupport/DbContextFactory.cs:32-45`, SQLite in-memory with `EnsureCreated`) but only 4 test files use it, all service-level.
- **Impact:** The tests most likely to catch real bugs (constraint violations, translation failures, filter interactions, broken migrations) are structurally incapable of doing so. InMemory also happily executes queries SQL Server would reject, so green integration tests do not imply the API works against the real provider.
- **Recommendation:** Add a SQLite-backed variant of `CustomWebAppFactory` (swap `UseInMemoryDatabase` for the existing `DbContextFactory.CreateSqlite` connection pattern + `EnsureCreated`) and move the constraint-sensitive suites (courses/enrollment, subscriptions, chat) onto it; add one dedicated test that runs the full migration chain against SQLite or LocalDB. Full parity needs SQL Server (Testcontainers) — overkill here, but SQLite closes most of the gap for effort M.
...
### TEST-5: Production schema was bent to the test provider: DateTimeOffset stored as strings so SQLite can sort  [Medium] [Effort: M]
- **Evidence:** `src/ResetYourFuture.Infrastructure/Data/ApplicationDbContext.cs:62-71` — `ConfigureConventions` applies `DateTimeOffsetToStringConverter` to **all** `DateTimeOffset` properties, with the comment citing SQLite's inability to translate DateTimeOffset ordering; the conversion applies equally to SQL Server production, where native `datetimeoffset` support exists.
- **Impact:** A testability workaround dictates production storage: date columns become NVARCHAR (range queries work only because ISO-8601 sorts lexically; type safety, date arithmetic in SQL, and index density are all degraded — DB 30 owns those consequences). The tradeoff was rational (it unlocked `DbContextFactory.CreateSqlite` for services using ordering) but it inverts the usual rule that tests adapt to production, not vice versa.
- **Recommendation:** Scope the conversion to the SQLite provider only — `ConfigureConventions` can branch on `Database.ProviderName`, or the TestSupport factory can apply the converter via its own `DbContextOptions`/derived context — then add a migration restoring native `datetimeoffset` on SQL Server. Coordinate with DB (30) before changing column types.
...
|----|----------|--------|--------|
| TEST-1 | High | M | SQLite-backed factory variant for constraint-sensitive suites; one migration-chain test |
...
| TEST-5 | Medium | M | Scope the DateTimeOffset→string converter to SQLite; restore native type on SQL Server (with DB 30) |
...
- **DB (30)** owns the schema consequences of the DateTimeOffset string storage (TEST-5) and of migrations never running under test (TEST-1).
