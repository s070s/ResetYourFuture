...
- **DB (30)** owns the schema consequences of the DateTimeOffset string storage (TEST-5); migrations previously ran under no test (TEST-1, now fixed — the chain is verified against LocalDB by `MigrationChainTests`).
