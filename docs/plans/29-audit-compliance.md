...
> **Fixed since audit:** COMP-1 (High — consent captured against no policy) — added bilingual `/privacy` (Privacy Policy) and `/terms` (Terms of Service) pages backed by a new `LegalRes` resource set (EN + EL, hand-edited Designer), reflecting the COMP-8 data inventory, the special-category assessment handling, retention, rights, and essential-cookies posture. The registration consent checkbox now carries a help line — "By registering, you agree to our Privacy Policy and our Terms of Service" — linking both pages, and the public landing footer links them too. Verified live in EN and EL. The pages are honest about being a non-commercial university project and note they are not legal advice. COMP-2 (High — special-category assessment answers stored in plaintext) — `AssessmentSubmission.AnswersJson`/`SummaryJson` are now encrypted at rest with ASP.NET Core Data Protection via a transparent EF Core value converter (`EncryptedStringConverter`), keyed off the existing DB-persisted key ring. The plaintext no longer touches the database file, an unfiltered backup, or an admin's raw table view — only the application can read it; the single write path and all three read paths (student history, admin submissions, assistant tool — which never reads answers anyway) go through the converter, and `string`→`string` keeps the `nvarchar` column so no migration was needed. Reads tolerate legacy plaintext rows. Special-category classification and disclosure are covered by COMP-1's privacy page; admin access stays role-gated (`AdminOnly`). Verified by two SQLite integration tests (ciphertext at rest, decrypt on read, legacy tolerance) exercising the real DataProtection stack, plus a clean SQL Server startup with no schema drift. Residual, lower-severity enhancements — a *separate* granular consent toggle and admin-read audit logging — are tracked under COMP-7.
